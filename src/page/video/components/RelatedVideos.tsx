...
export default function RelatedVideos({ videoId }: { videoId: string }) {
  ...
  return (
    <div>
      {isLoading ? Array.from({ length: 8 }, (_, i) => <VideoCardSkeleton key={i} />) : videos.map((v: VideoLightModel) => <VideoCard key={v.id} video={v} />)}
      {!isLoading && videos.length === 0 && <p className="text-[var(--text-muted)] text-sm">No related videos found.</p>}
    </div>
  );
}
