...
export default function VideoPlayer({ src }: { src: string }) {
  ...
  const toggleMute = () => {
    const next = !muted;
    setMuted(next); vid().muted = next;
  };

  /* ─── speed ─── */
  const setPlaybackSpeed = (s: number) => {
    setSpeed(s); vid().playbackRate = s; setShowSpeed(false);
  };
  ...
  return (
    <div
      ref={wrapRef}
      style={{ ...wrapperStyle, ...(theaterMode && !isFullscreen ? { maxWidth: "none", borderRadius: 0 } : {}) }}
    ...
    >
      ...
      {!playing && (
        <div
          ...
          onClick={togglePlay}
        >
          <div style={{ background: "rgba(0,0,0,.55)", borderRadius: "50%", width: 64, height: 64, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {/* Controls bar */}
      <div style={{ ...controlsBar, opacity: showControls || !playing ? 1 : 0 }} onClick={(e) => e.stopPropagation()}>
        {/* Play/Pause */}
        ...
        {/* Time */}
        <span style={timeStyle}>{fmt(currentTime)} / {fmt(duration)}</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          {/* Speed */}
          <button style={{ ...btnStyle, fontSize: 13, fontWeight: 600 }} onClick={() => setShowSpeed(!showSpeed)} title="Playback speed">
            {speed}x
          </button>

          {/* Theater */}
          <button style={btnStyle} onClick={() => setTheaterMode(!theaterMode)} title="Theater mode">
      ...
      {showSpeed && (
        <div style={speedMenuStyle} onClick={(e) => e.stopPropagation()}>
          {SPEEDS.map((s) => (
            <div key={s} style={speedItemStyle(s === speed)} onClick={() => setPlaybackSpeed(s)} onMouseEnter={(e) => (e.currentTarget.style.background = "#333")} onMouseLeave={(e) => (e.currentTarget.style.background = s === speed ? "#333" : "transparent")}>
              {s === 1 ? "Normal" : `${s}x`}
            </div>
          ))}
        </div>
